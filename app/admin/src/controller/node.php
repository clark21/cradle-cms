...
/**
 * Render dynamic node update
 * 
 * @param Request $request
 * @param Response $response
 */
$cradle->get('/admin/node/:node_type/update/:node_id', function($request, $response) {
    //----------------------------//
    // 1. Route Permissions
    //only for admin
    cradle('global')->requireLogin('admin');
    ...
    if(empty($data['item'])) {
        ...
        // merge meta with item
        $data['item'] = array_merge($data['item'], $data['item']['node_meta']);
    }
    ...
    $class = sprintf(
        'page-developer-node-%s-update page-admin',
        $meta['meta_key']
    );
...
/**
 * Process dynamic node update
 * 
 * @param Request $request
 * @param Response $response
 */
$cradle->post('/admin/node/:node_type/update/:node_id', function($request, $response) {
    //----------------------------//
    // 1. Route Permissions
    //only for admin
    cradle('global')->requireLogin('admin');
    ...
    if(!empty($errors)) {
        ...
        return cradle()
            ->triggerRoute(
                'get',
                sprintf(
                    '/admin/node/%s/update/%s', 
                    $request->getStage('node_type'),
                    $request->getStage('node_id')
                ),
                $request,
                $response
            );
