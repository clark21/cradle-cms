<?php //-->
/**
 * This file is part of a Custom Project
 * (c) 2017-2019 Acme Inc
 *
 * Copyright and license information can be found at LICENSE.txt
 * distributed with this package.
 */

use Cradle\Module\Meta\Validator;

/**
 * Validator layer test
 *
 * @vendor   Acme
 * @package  Meta
 * @author   John Doe <john@acme.com>
 */
class Cradle_Module_Meta_ValidatorTest extends PHPUnit_Framework_TestCase
{
    /**
     * @covers Cradle\Module\Meta\Validator::getCreateErrors
     */
    public function testGetCreateErrors()
    {
        $actual = Validator::getCreateErrors([]);
        $this->assertEquals('Singular is required', $actual['meta_singular']);
        $this->assertEquals('Plural is required', $actual['meta_plural']);
        $this->assertEquals('Slug is required', $actual['meta_slug']);
    }

    /**
     * @covers Cradle\Module\Meta\Validator::getUpdateErrors
     */
    public function testGetUpdateErrors()
    {
        $actual = Validator::getUpdateErrors([]);

        $this->assertEquals('Invalid ID', $actual['meta_id']);
    }
}
